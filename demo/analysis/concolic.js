...
// module.exports = (script, path) => eval(instrument(script, null, path));

// END VERBATIM

const ljson = LinvailJSON(linvail); // JSON must be shared because of randomly generated prefix!!!

require("drizzt")({
  traps: META,
  sync:true,
  emitter: process.emitter.trace("META"),
  instrument: (script, source, origin) => instrument(script, null, source),
  initialize: (global, origin) => {
    global.JSON = ljson;
    return {
      namespace: "META",
      setup: Astring.generate(aran.setup(pointcut)),
      sandbox: linvail.base.flip(global)
    };
  }
}, (error) => {
